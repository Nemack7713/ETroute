...
class P4aRecipeSubsystemTests(unittest.TestCase):
    ...
    def test_pure_python_uses_pip(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            inspection = self.classifier.inspect(tmp, "demo")
            plan = self.planner.plan(inspection, ["arm64-v8a"])
            self.assertEqual(inspection.kind, PackageKind.PURE_PYTHON)
    ...
    def test_pyproject_uses_pyproject_recipe(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            inspection = self.classifier.inspect(tmp, "modern")
            plan = self.planner.plan(inspection, ["arm64-v8a"])
            self.assertEqual(inspection.kind, PackageKind.PYPROJECT)
    ...
    def test_native_extension_uses_compiled_components(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            inspection = self.classifier.inspect(tmp, "native_demo")
            plan = self.planner.plan(inspection, ["arm64-v8a"])
            self.assertEqual(inspection.kind, PackageKind.COMPILED_EXTENSION)
    ...
    def test_android_mk_uses_ndk_recipe(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            inspection = self.classifier.inspect(tmp, "ndk_demo")
            plan = self.planner.plan(inspection, ["arm64-v8a"])
            self.assertEqual(inspection.kind, PackageKind.NDK)
    ...
    def test_backend_generates_private_recipe_and_manifest(self):
        with tempfile.TemporaryDirectory() as tmp, tempfile.TemporaryDirectory() as output:
            Path(tmp, "engine.pyx").write_text("cdef int value = 1\n", encoding="utf-8")
            result = P4aCapabilityBackend().resolve(
                "p4a.recipe.generate",
                {
                    "project": tmp,
                    "package": "engine-demo",
